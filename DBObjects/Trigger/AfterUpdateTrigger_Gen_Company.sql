CREATE TRIGGER AfterUpdateTrigger_Gen_Company
ON gen_company
AFTER UPDATE
AS
BEGIN
    IF UPDATE(CompanyTypeID) OR UPDATE(IsTransparentPricing) OR UPDATE(AM_SalesPersonID) OR UPDATE(NBD_SalesPersonID)
	BEGIN
		DECLARE 
